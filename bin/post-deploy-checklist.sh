...
printf '----------------------------------\n'
go=$(curl -sI "$PROD_URL/go?u=https://example.com" | awk 'NR==1{print $2}' || echo 000)
go_deal=$(curl -sI "$PROD_URL/go-deal?u=https://example.com" | awk 'NR==1{print $2}' || echo 000)
printf '   /go: %s\n   /go-deal: %s\n' "$go" "$go_deal"
...
printf '--------------------------------\n'
admin=$(curl -sI "$PROD_URL/wp-admin/" | awk 'NR==1{print $2}' || echo 000)
printf '   /wp-admin/: %s\n' "$admin"
...
printf '-----------------\n'
if ssh "$DEPLOY_HOST" "sudo tail -n 50 /var/log/nginx/error.log 2>/dev/null || sudo tail -n 50 /var/log/apache2/error.log 2>/dev/null" | grep -E '(critical|fatal|PHP Fatal)' >/tmp/error-scan.$$.log 2>/dev/null; then
