...
The shared cookie-touch predicate excludes query REST and subdirectory
`wp-json` paths while preserving the legacy query override. Public owner-grant
issuance prunes expired pending rows in a bounded transaction and enforces the
256-row pending quota before token/cookie creation; verified recovery bypasses
only the public quota. The lock/read is bounded concurrency control under the
normal InnoDB isolation contract; non-default isolation remains a staging
verification gate. No provider, remote, database, cron or watchdog authority
is added.
