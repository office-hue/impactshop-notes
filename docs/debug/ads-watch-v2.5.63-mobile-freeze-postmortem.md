# ads-watch — ~2 hetes Freeze Saga Postmortem (2026-03-26 → 2026-04-10)
...
**Végső verzió:** v2.5.64 (commit `ca6f087a`, 2026-04-10) — iOS ytDur=0 gap fix  
...
**Bug rétegek száma:** 6 különálló hiba (v2.5.64: iOS/iPhone-specifikus új réteg)
...
| **v2.5.64** | **2026-04-10** | **iOS ytDur=0 gap fix — ha getDuration()=0, 180s fallback deadline** | **iPhone iOS Safari** |

---

## 12. Bug réteg #6 — iOS ytDur=0 deadline gap (iPhone iOS Safari)

### Kiváltó ok

A v2.5.63 bevezette a `sponsorYoutubeDeadlineId` hard deadline mechanizmust:
```javascript
if (ytDur > 0) {
    state.sponsorYoutubeDeadlineId = window.setTimeout(...);
}
```
Az `if (ytDur > 0)` guard hibás volt: iOS WebKit (iPhone Safari) a `PLAYING` eventet **korábban tüzeli** mint ahogy a `getDuration()` érvényes értéket adna vissza.

### Tünet

- **iPhone iOS Safari**: `getDuration()` a `PLAYING` pillanatában `0`-t ad vissza
- `if (ytDur > 0)` → guard megakadályozza a deadline beállítást
- ENDED event iOS-on háttér-throttling esetén elmarad (v2.5.63 által javított probléma)
- De ha `ytDur=0` → nincs backup deadline → ugyanaz a végtelen freeze tér vissza iOS-on

**Android Chrome-on és Desktop-on nem reprodukálható** — ott `getDuration()` a `PLAYING`-ra már helyes értéket ad.

### Fix — v2.5.64 (commit `ca6f087a`, 2026-04-10)

```javascript
// RÉGI (v2.5.63) — hiányos iOS-en:
if (event.data === YT.PlayerState.PLAYING) {
    var ytDur = Number(event.target.getDuration() || 0);
    if (ytDur > 0) {
        state.sponsorYoutubeDeadlineId = window.setTimeout(function () { ... }, ytDur * 1000 + 3000);
    }
    // HA ytDur=0 → semmi deadline → iOS freeze
}

// ÚJ (v2.5.64) — mindig van deadline:
if (event.data === YT.PlayerState.PLAYING) {
    var ytDur = Number(event.target.getDuration() || 0);
    var deadlineSec = ytDur > 0 ? ytDur + 3 : 180; // 180s fallback ha getDuration még nem kész
    state.sponsorYoutubeDeadlineId = window.setTimeout(function () {
        state.sponsorYoutubeDeadlineId = null;
        if (!state.sponsorCompletionFired && state.isPlaying && state.currentMode === 'sponsor') {
            state.sponsorCompletionFired = true;
...
- **iPhone iOS Safari**: fix után rendben — Safari-n a flow lefut, jutalom kiosztva ✅
- **Chrome iOS (iPhone Chrome)**: még vizsgálat alatt — Chrome iOS a JS contextet teljes egészében suspend-eli backgroundban; a `setTimeout` sem fut le suspend alatt → ezt a `visibilitychange` + `handleExternalReturn` flow nem kezeli ha `externalNavigationPending !== true`
...
HOTFIX_ALLOW_PHP_MISMATCH=1 \
IMPACTSHOP_BASTION_WRITE_ALLOW=YES \
IMPACTSHOP_BASTION_WRITE_APPROVAL=ads-watch-v2.5.64-ios-ytdur0-fix \
IMPACTSHOP_BASTION_ALLOW_PATHS='wp-content/mu-plugins/impactshop-ads-watch.js' \
bash scripts/hotfix-sync.sh wp-content/mu-plugins/impactshop-ads-watch.js
```

**MD5 (szerver):** `8cf62e07f108593593f98b74f03a24f7`  
**Branch:** `hotfix/mobile-freeze-v2.5.63`

### Nyitott: Chrome iOS freeze

- **Hipotézis:** Chrome iOS `visibilitychange` → `handleVisibilityChange()` → `if (!document.hidden) handleExternalReturn()` — de `state.externalNavigationPending` csak akkor `true` ha felhasználó external linket nyomott; általános háttérbe menésnél `false` → `handleExternalReturn()` nem állítja helyre a state-et megfelelően.
- **Következő lépés:** `handleVisibilityChange()` kiegészítése: ha `!document.hidden && state.isPlaying && (state.currentMode === 'sponsor' || state.currentMode === 'auto_banner') && !state.externalNavigationPending` → recovery kényszerítése.
...
7. **`getDuration()` iOS race condition** — iOS WebKit `PLAYING` event és `getDuration()` aszinkron: `getDuration()` `0`-t ad vissza amíg a player nem töltötte be teljesen a videót. Soha ne legyen `if (ytDur > 0)` guard a deadline beállítás körül — mindig kell fallback érték (`180s` vagy más ésszerű maximum).

8. **Platform-specifikus JS suspend (Chrome iOS)** — Chrome iOS (ellentétben Safarival) teljes JS contextet suspend-el background állapotban; `setTimeout`, `setInterval` sem fut le. A recovery mechanizmusnak explicit visibility-return mód kell, nem csak `externalNavigationPending` alapú detekció.
