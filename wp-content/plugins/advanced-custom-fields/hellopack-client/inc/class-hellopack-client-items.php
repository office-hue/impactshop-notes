...
if ( ! class_exists( 'HelloPack_Client_Items' ) ) :

	/**
	 * Creates the theme & plugin arrays & injects API results.
	 *
	 * @class HelloPack_Client_Items
	 * @version 1.0.0
	 * @since 2.0.0
	 */
	class HelloPack_Client_Items {
		 ...
		 *
		 * @see HelloPack_Client_Items()
		 * @uses HelloPack_Client_Items::init_actions() Setup hooks and actions.
		 *
		 * @since 2.0.0
		 * @static
		 * @return object The one true HelloPack_Client_Items.
		 * @codeCoverageIgnore
		 */
		 ...
		 *
		 * @see HelloPack_Client_Items::instance()
		 *
		 * @since 2.0.0
		 * @access private
		 * @codeCoverageIgnore
		...
		public function init_actions() {
			...
			add_filter( 'site_transient_update_plugins', array( $this, 'update_plugins_transit' ), PHP_INT_MAX, 1 );
			add_filter( 'transient_update_plugins', array( $this, 'update_plugins_transit' ), PHP_INT_MAX, 1 );

			// Inject theme updates into the response array.
			add_filter( 'pre_set_site_transient_update_themes', array( $this, 'update_themes' ), PHP_INT_MAX, 1 );
			add_filter( 'pre_set_transient_update_themes', array( $this, 'update_themes' ), PHP_INT_MAX, 1 );

			// Inject plugin information into the API calls.
			add_filter( 'plugins_api', array( $this, 'plugins_api' ), PHP_INT_MAX, 3 );
			...
			add_action( 'deleted_plugin', array( $this, 'rebuild_plugins' ) );
		}
		 ...
		 * @return object
		 */
		public function update_plugins_transit( $transient ) {
			self::set_plugins( false );
			...
			foreach ( $installed as $plugin => $premium ) {
				if ( isset( $plugins[ $plugin ] ) && version_compare( $plugins[ $plugin ]['Version'], $premium['version'], '<' ) ) {
					$_plugin = array(
						...
						'url'         => $premium['url'],
						'package'     => hellopack_client()->api()->deferred_downloads( $premium['id'] ),
					);
					// $transient->response[ $plugin ] = (object) $_plugin;
				} else {
					// TODO: BETA function, needs testing.
			...
			if ( 'plugin_information' === $action && isset( $args->slug ) ) {
				...
				foreach ( $installed as $slug => $plugin ) {
					if ( dirname( $slug ) === $args->slug ) {
						...
						$response->download_link  = hellopack_client()->api()->deferred_downloads( $plugin['id'] );
						break;
					}
				}
			}
		...
		public function set_themes( $forced = false, $use_cache = false ) {
			$themes_transient = get_site_transient( hellopack_client()->get_option_name() . '_themes' );
			self::$themes     = is_array( $themes_transient ) ? $themes_transient : array();

			if ( empty( self::$themes ) || true === $forced ) {
				$themes = hellopack_client()->api()->themes();
				foreach ( hellopack_client()->get_option( 'items', array() ) as $item ) {
					...
					if ( 'theme' === $item['type'] ) {
						...
						$request      = hellopack_client()->api()->item( $item['id'], $request_args );
						if ( false !== $request ) {
							$themes[] = $request;
						}
					}
		...
		public function set_plugins( $forced = false, $use_cache = false, $args = array() ) {
			$plugins_transient = get_site_transient( hellopack_client()->get_option_name() . '_plugins' );
			self::$plugins     = is_array( $plugins_transient ) ? $plugins_transient : array();

			if ( empty( self::$plugins ) || true === $forced ) {
				$plugins = hellopack_client()->api()->plugins();
				foreach ( hellopack_client()->get_option( 'items', array() ) as $item ) {
					if ( empty( $item ) ) {
						continue;
					}
		...
		private function process_themes( $purchased ) {
			...
			set_site_transient( hellopack_client()->get_option_name() . '_themes', self::$themes, HOUR_IN_SECONDS );
		}
		...
		private function process_plugins( $purchased, $args = array() ) {
			...
			set_site_transient( hellopack_client()->get_option_name() . '_plugins', self::$plugins, HOUR_IN_SECONDS );
		}
	}
