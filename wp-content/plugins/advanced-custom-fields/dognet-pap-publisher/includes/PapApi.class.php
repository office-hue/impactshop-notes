 ...
 *   Generated on: 2025-07-25 04:24:55
 *   PAP version: 5.14.22.1, GPF version: 1.3.69.0
 *   
 */
...
if (!class_exists('Gpf_Http', false)) {
  class Gpf_Http extends Gpf_Object implements Gpf_HttpResponse {
      ...
      public static function getRequestVariable($name) {
          /* @phpstan-ignore function.alreadyNarrowedType (until $_REQUEST type is explicitly specified) */
      ...
      public static function setCspHeader(): void {
          if (Gpf_Settings::get(Gpf_Settings_Gpf::CSP_ENABLE_HEADER) != Gpf::YES) {
              return;
          }
  
          $arrayData = Gpf_Csp_CspHeaderData::getApplicationPoliciesArray();
          $arrayData = Gpf_Csp_CspDomainsUtil::addCspTrustedDomainsFromSetting($arrayData, Gpf_Settings_Gpf::CSP_TRUSTED_DOMAINS);
          $builder = new Gpf_Csp_CspBuilder($arrayData);
          Gpf_Csp_CspDomainsUtil::addOriginalServerDomain($builder);
          Gpf_Csp_CspDomainsUtil::addPluginDomains($builder);
          $result = $builder->getCompiledHeader();
          self::setHeader('Content-Security-Policy', $result);
      }
...
if (!class_exists('Gpf_Data_Record', false)) {
  class Gpf_Data_Record extends Gpf_Object implements Iterator, Gpf_Rpc_Serializable,
       ...
       * Create record
       *
       * @param array|Gpf_Data_RecordHeader $header
...
if (!class_exists('Gpf_Net_Http_Exception', false)) {
  class Gpf_Net_Http_Exception extends Gpf_Exception {
      ...
      public function __construct($message, $httpResponseCode = 0, $httpResponseBody = null) {
...
if (!class_exists('Gpf_Log', false)) {
  class Gpf_Log  {
      ...
      public static function serverLogWarning(string $message): void {
          self::serverLog($message, E_USER_WARNING);
      }
  
      public static function serverLogNotice(string $message): void {
          self::serverLog($message, E_USER_NOTICE);
      }
  
      private static function serverLog(string $message, int $errorLevel): void {
          $hostedAccountId = Gpf_Paths::getInstance()->getServerHostedAccountId();
          trigger_error('AccountId: '.$hostedAccountId . ' ' . $message, $errorLevel);
      }
       ...
       */
      public static function log($message, $logLevel, $logGroup = null): void {
...
if (!class_exists('Gpf_Log_Logger', false)) {
  class Gpf_Log_Logger extends Gpf_Object {
       ...
       * @param string $logGroup
       */
      public function log($message, $logLevel, $logGroup = null): void {
          $time = time();
          $group = $logGroup;
          if($this->group !== null) {
              $group = $this->group;
  ...
  class Pap_Api_Tracker extends Gpf_Object {
      ...
      private function parseResponse(): void {
          if (
              ...
              || !preg_match('/setVisitor\(\'([a-zA-Z0-9]+)\'\);/', $this->trackingResponse, $matches)
          ) {
              return;
          }
