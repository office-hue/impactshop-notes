(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent =
      ...
      ".impact-auction-widget__gallery{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}" +
      ".impact-auction-widget__card{display:grid;gap:10px;padding:12px;border-radius:18px;border:1px solid rgba(255,255,255,.11);background:rgba(9,19,55,.62);cursor:pointer;transition:transform .18s ease,border-color .18s ease,background .18s ease}" +
      ...
      ".impact-auction-widget__image{aspect-ratio:4/5;border-radius:14px;overflow:hidden;background:rgba(255,255,255,.08)}" +
      ...
      ".impact-auction-widget__price{font-size:18px;font-weight:800}" +
      ...
      ".impact-auction-widget__detail-item{padding:10px 12px;border-radius:14px;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.08)}" +
      ...
      ".impact-auction-widget__preset{appearance:none;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);color:var(--iaw-text);border-radius:12px;padding:10px 12px;font-weight:700;cursor:pointer}" +
      ...
      "@media (max-width:900px){.impact-auction-widget__stats{grid-template-columns:repeat(2,minmax(0,1fr))}.impact-auction-widget__gallery{grid-template-columns:repeat(2,minmax(0,1fr))}}" +
      "@media (max-width:640px){.impact-auction-widget{padding:18px;border-radius:24px}.impact-auction-widget__title{font-size:32px}.impact-auction-widget__stats{grid-template-columns:1fr}.impact-auction-widget__gallery{grid-template-columns:1fr}.impact-auction-widget__row,.impact-auction-widget__detail-grid,.impact-auction-widget__presets{grid-template-columns:1fr}.impact-auction-widget__drawer{padding:0}.impact-auction-widget__panel{width:100%;height:100%;border-radius:0}}";
  ...
  function createMarkup() {
    return (
      '<section class="impact-auction-widget" role="region" aria-label="Jovonk Vize aukcio widget">' +
      '<div class="impact-auction-widget__inner">' +
      '<span class="impact-auction-widget__eyebrow">Gala aukcio widget</span>' +
      '<h3 class="impact-auction-widget__title" data-role="title">Jovonk Vize gala aukcio</h3>' +
      '<p class="impact-auction-widget__subtitle" data-role="subtitle">Miele mutargyak es felajanlasok</p>' +
      '<p class="impact-auction-widget__desc" data-role="description">Additiv frontend lane a Jovonk Vize aukcios flow-hoz.</p>' +
      '<div class="impact-auction-widget__stats">' +
      '<div class="impact-auction-widget__stat"><div class="impact-auction-widget__stat-label">Kampany paid total</div><div class="impact-auction-widget__stat-value" data-role="stat-combined">0 Ft</div></div>' +
      ...
      '<div class="impact-auction-widget__stat"><div class="impact-auction-widget__stat-label">Lezart lotok</div><div class="impact-auction-widget__stat-value" data-role="stat-closed-lots">0</div></div>' +
      '<div class="impact-auction-widget__stat"><div class="impact-auction-widget__stat-label">Tetelszam</div><div class="impact-auction-widget__stat-value" data-role="stat-lots">0</div></div>' +
      '</div>' +
      ...
      '<button type="button" class="impact-auction-widget__close" data-role="close">Bezaras</button>' +
      '</div>' +
      ...
      '<div class="impact-auction-widget__detail-item"><small>Megjelenitett ar</small><span data-role="detail-price"></span></div>' +
      '<div class="impact-auction-widget__detail-item"><small>Minimum kovetkezo licit</small><span data-role="detail-next-bid"></span></div>' +
      '<div class="impact-auction-widget__detail-item"><small>Technika</small><span data-role="detail-medium"></span></div>' +
      '<div class="impact-auction-widget__detail-item"><small>Meret</small><span data-role="detail-dimensions"></span></div>' +
      '</div>' +
      ...
      '<input class="impact-auction-widget__input" data-role="bid-amount" type="text" inputmode="numeric" placeholder="Licitar (Ft)">' +
      '<div class="impact-auction-widget__row">' +
      '<input class="impact-auction-widget__input" data-role="bid-email" type="email" placeholder="E-mail cim" required>' +
      '<input class="impact-auction-widget__input" data-role="bid-phone" type="text" placeholder="Telefonszam (opcionalis)">' +
      '</div>' +
      '<input class="impact-auction-widget__input" data-role="bid-name" type="text" placeholder="Nev (opcionalis)">' +
      '<div class="impact-auction-widget__note">Email kotelezo. Az SMS lane kulon disclosure es kesobbi backend-bekotes utan aktivalhato.</div>' +
      '<button class="impact-auction-widget__submit" type="submit">Licit megerositese</button>' +
  ...
  function mountWidget(mountEl, config) {
    ...
    function detailOpen(lot) {
      ...
      els.detailPrice.textContent = (lot.display_label || "Ar") + ": " + (lot.display_amount_formatted || "");
      ...
      els.detailImage.innerHTML = lot.image_url
        ? '<img src="' + escapeHtml(lot.image_url) + '" alt="' + escapeHtml((lot.artist_name || "") + ' - ' + (lot.item_title || "")) + '">' 
        : "";
      renderPresets(lot);
      els.bidAmount.value = String((lot.display_amount || lot.starting_bid || 0) + (lot.min_increment || 0));
      setStatus(els.detailStatus, "A bidder regisztracio es a licit submit lane aktiv. A winner payment backend kesz, de a triggerelo admin UI kulon fazisban jon.", "");
    ...
    function renderLots(payload) {
      ...
      els.gallery.innerHTML = lots.map(function (lot) {
        return (
          ...
          '<div class="impact-auction-widget__image">' +
          (lot.image_url ? '<img src="' + escapeHtml(lot.image_url) + '" alt="' + escapeHtml((lot.artist_name || "") + ' - ' + (lot.item_title || "")) + '">' : '') +
          '</div>' +
    ...
    function renderPayload(payload) {
      ...
      els.title.textContent = payload.title || "Jovonk Vize gala aukcio";
      ...
      setStatus(els.status, payload.security && payload.security.write_enabled
        ? "A read API es az alap bidder/bid write lane aktiv. A winner payment backend kesz, de frontend/admin trigger UI meg nincs."
        : "A read API aktiv, de ezen az originen a write lane nincs engedelyezve.", (payload.security && payload.security.write_enabled) ? "success" : "");
    }

    function loadPublic() {
      return requestJson(apiBase + "/" + encodeURIComponent(campaign) + "/public").then(renderPayload).catch(function (error) {
        setStatus(els.status, "A public scaffold payload nem toltheto be: " + error.message, "error");
      });
    }
    ...
    els.bidForm.addEventListener("submit", function (event) {
      ...
      if (!state.activeLot) {
        setStatus(els.detailStatus, "Nincs aktiv tetel kivalsztva.", "error");
        return;
      }
      if (!els.bidEmail.value.trim()) {
        setStatus(els.detailStatus, "Az e-mail cim kotelezo a scaffold szerinti bidder flowban.", "error");
        return;
      }

      if (!state.sessionToken) {
        setStatus(els.detailStatus, "Ezen az originen nincs write session token. A licitkuldes nem engedelyezett.", "error");
        return;
      }
      ...
      setStatus(els.detailStatus, "Bidder regisztracio folyamatban...", "");
      ...
      requestJson(registerUrl, {
        ...
        setStatus(els.detailStatus, "Licit rogzitese folyamatban...", "");
        ...
        setStatus(els.detailStatus, "Licit rogzitve: " + (bidResponse.bid_amount_formatted || ""), "success");
        ...
        setStatus(els.detailStatus, "Licitkuldes sikertelen: " + (payload.error || error.message || "ismeretlen hiba"), "error");
      });
    });
