<?php
/**
 * Sharity – Banners-based deals (MU) + diagnosztika + last-good fallback
 * Shortcodes:
...
function sib_load_banners(){
  ...
  if($c===200 && $csv){
    ...
    foreach($lines as $i=>$ln){ if($ln==='') continue; $cols=str_getcsv($ln); if(!$i){ $hdr=$cols; continue; }
      ...
      $label=$rec['label']??[]; if(is_string($label)){ $d=json_decode($label,true); if(is_array($d)) $label=$d; }
      $out[]=[
        'slug'=>$slug,
        'href'=>$href,
  ...
  if ($out){ sib_store_last_good($out); return $out; }
  $fallback = sib_load_last_good();
...
function sib_sc_deals($atts){
  ...
  $force=$a['force']==='1'; $items=sib_load_banners(); if(!$items) return '<div>Jelenleg nincs megjeleníthető ajánlat.</div>';
  $catf=array_filter(array_map('trim', preg_split('~[|,]~',(string)$a['category'])));
  if($catf){ $items=array_filter($items,function($it)use($catf){ $hay=' '.strtolower((string)$it['category']).' '; foreach($catf as $n){ if($n!=='' && str_contains($hay, strtolower($n))) return true; } return false; }); }
  $items=array_slice(array_values($items),0,max(1,(int)$a['limit'])); $cards=[];
  foreach($items as $it){ $go=sib_build_deal_link($it['slug'],$it['href'],$force); $img=$it['img']?'<img src="'.esc_url($it['img']).'" alt="">':'<div class="ph"></div>'; $old=$it['old_price']?'<s class="old">'.esc_html($it['old_price']).'</s>':''; $pr =$it['price']?'<span class="price">'.esc_html($it['price']).'</span>':''; $pct=(int)$it['pct']>0?'<span class="badge">-'.(int)$it['pct'].'%</span>':''; $cat=$it['category']?'<div class="cat">'.esc_html($it['category']).'</div>':''; $cards[]='<a class="imx-deal" href="'.esc_url($go).'" target="_blank" rel="nofollow sponsored noopener">'.'<div class="thumb">'.$img.'</div>'.'<div class="body"><div class="title">'.esc_html($it['title']).'</div>'.'<div class="row">'.$old.$pr.$pct.'</div>'.$cat.'</div>'.'</a>'; }
  $css='<style>.imx-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:12px}.imx-deal{display:flex;gap:.75rem;padding:.75rem;border:1px solid #eee;border-radius:14px;text-decoration:none;color:inherit;background:#fff;transition:transform .12s,box-shadow .12s}.imx-deal:hover{transform:translateY(-2px);box-shadow:0 4px 16px rgba(0,0,0,.06)}.imx-deal .thumb{flex:0 0 auto}.imx-deal img,.imx-deal .ph{width:88px;height:88px;object-fit:cover;border-radius:10px;background:#f7f7f7;display:block}.imx-deal .title{font-weight:600;line-height:1.25;margin-bottom:.25rem}.imx-deal .row{display:flex;align-items:center;gap:.5rem}.imx-deal .old{opacity:.6;text-decoration:line-through}.imx-deal .badge{font-size:.85em;padding:.15rem .4rem;border-radius:6px;background:#ffe8e8}.imx-deal .cat{font-size:.8em;opacity:.7;margin-top:.25rem}</style>';
  return $css.'<div class="imx-grid">'.implode('', $cards).'</div>';
}
add_shortcode('impactshop_deals_banners','sib_sc_deals');
add_action('init', function(){ global $shortcode_tags; if (empty($shortcode_tags['impactshop_deals'])) add_shortcode('impactshop_deals','sib_sc_deals'); }, 1);
