...
if (!function_exists('impactshop_full_leaderboard_fetch_totals')) {
    function impactshop_full_leaderboard_fetch_totals(): array
    {
        $url = home_url('/wp-json/impact/v1/ticker');
        $res = wp_remote_get($url, [
            'timeout' => 20,
            'headers' => ['Accept' => 'application/json'],
        ]);
...
if (!function_exists('impactshop_full_leaderboard_is_unknown_ngo')) {
    function impactshop_full_leaderboard_is_unknown_ngo(string $name): bool
    {
...
if (!shortcode_exists('impact_full_leaderboard')) {
    add_shortcode('impact_full_leaderboard', function ($atts) {
        $default_from = '2025-10-23';
        $default_rate = (float)get_option('impactshop_huf_rate', 392);
        if ($default_rate <= 0) {
            $default_rate = 392;
        }
        $a = shortcode_atts([
        ...
        if (is_array($ngo)) {
            $ngo = array_values(array_filter($ngo, function ($row) {
                $name = isset($row['name']) ? (string)$row['name'] : '';
                return !impactshop_full_leaderboard_is_unknown_ngo($name);
            }));
        }
        ...
        if (strtolower($a['layout']) === 'rich') {
            ...
            $totals = impactshop_full_leaderboard_fetch_totals();
