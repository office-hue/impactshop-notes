# Google Sheets AppScript Kódok

Ez a mappa tartalmazza a Google Sheets AppScript kódokat, amik a banner/webshop adatok automatikus frissítéséért felelősek.

## Spreadsheet-ek:

- **Shops**: https://docs.google.com/spreadsheets/d/17EPyzCz5YPGpqw6fStgbPD07ZZAniZjIeaaE0iMfhfE/edit?pli=1&gid=0#gid=0
- **Banners**: https://docs.google.com/spreadsheets/d/17EPyzCz5YPGpqw6fStgbPD07ZZAniZjIeaaE0iMfhfE/edit?pli=1&gid=328401803#gid=328401803  
- **Banners_tmp**: https://docs.google.com/spreadsheets/d/17EPyzCz5YPGpqw6fStgbPD07ZZAniZjIeaaE0iMfhfE/edit?pli=1&gid=470200079#gid=470200079

## Fájlok:

- `Code.gs` - **Főmotor (v6.7)**: Patrol rendszer promo-preferenciával, JSON label payload-dal, multi-séma támogatással (Árukereső/Google/Generic)
- `ArukeresoRunners.gs` - **Árukereső-specifikus runner**: Dedikált Árukereső XML feldolgozó, chunk-alapú parsing 
- `api-banners.gs` - **JSON API**: Banners sheet → JSON végpont, egységesített objektum struktúrával

## Banner Link Hiba:

**🔍 PROBLÉMA AZONOSÍTVA**: Az akciós bannerek linkje NEM megy át az ImpactShop redirect rendszeren!

**Érintett kód részlet** (`impactshop_scroller`):
```php
<a href="<?php echo esc_url($b['href']); ?>">
```

**Mi a baj:**
1. ❌ A banner `href` közvetlenül a CSV-ből jön
2. ❌ NEM használja a `/go/` vagy `/go-deal/` redirect rendszert
3. ❌ NEM rögzíti a konverziót Dognet API-n
4. ❌ NEM adja át a `d1` paramétert (NGO tracking)

**Javítás szükséges:**
A banner linkeket is át kell vezetni a redirect rendszeren, mint a shop linkeket:
```php
$banner_cta = $d1
  ? add_query_arg(['d1'=>$d1,'amb'=>$amb,'src'=>$src], home_url('/go-deal/'. rawurlencode($shop_slug) .'?u='. rawurlencode($product_url)))
  : $b['href'];
```
