...
- Brevo (hírlevél): API kulcs `~/.impact-secrets/env.d/capi.env` (`BREVO_API_KEY`),
  küldő adatok `~/.impact-secrets/env.d/ai-agent.env` (`BREVO_SENDER_EMAIL`,
  `BREVO_SENDER_NAME`).
- Árukereső autobanner (ads-watch) végleges flow: **Fillout → /go-deal → Dognet deeplink**.
  A kliens‑oldali interceptort tiltsuk (`impact-arukereso-deeplink-fix.php.off`), és
  a deeplink guardok ne ürítsék a `u` paramot.
- JYSK szavazás: végleges UI/flow (mobil CTA + egymezős NGO kereső + ID panel nélkül),
  odds képlet 3 nyereménnyel, sorsolás max 3 nyertes.
- Cégjelző API: `impactshop-cegjelzo.php` + REST `impact/v1/cegjelzo/ngo-registry`,
  heti sync cron, védett fájl.
- Hírlevél szolgáltatás: Brevo kulcs + sender env csak secrets-ben; nem kerülhet repo-ba.
- ID widget + ID panel: `impactshop-identity-panel.php/.js` védett; JYSK vote modulból kivéve,
  csak profil oldalon panel, a widget marad.
