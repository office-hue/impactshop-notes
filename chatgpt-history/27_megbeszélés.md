...
Ezt mondtad:
Ebben a Tim Guard időtartamot növelni kell mert több lett a webshop. Mi ennek a módja: /**
...
Gondolkozás
A “TIME-GUARD” nálad a MAX_RUN_MS limitből jön: ha a futás kezdete óta eltelt idő nagyobb ennél, a script leáll („folyt. következő körben”). Több webshopnál egyszerűen növeld ezt az értéket — és igény szerint igazítsd a többi időzítőt is.
Itt a javasolt, biztonságos (Apps Script ~6 perc körüli plafonhoz igazított) beállításcsomag egyben, csak cseréld a konstansokat a snippet tetején:

// futási korlátok — TIM GUARD és társai (nagyobb katalógushoz hangolva)
const SHOPS_PER_RUN     = 18;      // volt: 10  — több bolt / kör
const MAX_RUN_MS        = 280000;  // volt: 220000 — ~4m40s (6 perc limit alatt marad)
...
Miért így?
MAX_RUN_MS 280s: növeli a körben feldolgozható boltok számát, de még bőven a ~6 perces Apps Script futási plafon alatt marad, így nem fog „Execution time exceeded”-et dobni.
SHOPS_PER_RUN 18: nagyobb lépésenkénti mennyiség. Ha látod, hogy még bőven van idő a végén, feljebb veheted 20–24-ig.
PER_FEED_MS 26s: időt hagy a „nagy” vagy lassabb feedek feldolgozására (különösen Árukereső → chunk-parse).
PREFLIGHT_MS 4s és SLEEP_BETWEEN 60ms: egy kicsit agresszívabb, gyorsabb köröket enged (ha bármelyik partner throttle-ol, ezt visszaemelhetjük).
További finomhangolási tippek (opcionális, de hasznos nagy katalógusnál):
Ha egyes feedek ~0.5–1s alatt bejönnek és egyszerűek, csökkentheted a PER_FEED_MS-t 20000-re, és inkább emelheted a SHOPS_PER_RUN-t. A snippetben már van dinamikus “remain” számolás, ezt kihasználja.
Ha a futások néha mégis plafonba futnak, vedd MAX_RUN_MS = 300000-ra (pont 5 perc), és SHOPS_PER_RUN-t 15–16 körülire.
Ha a teljes katalógus nagy, nem gond, mert a cursor (PS_KEY_CURSOR) miatt több körben szépen „végigtolja”.
Operatív lépések:
Fentieket beilleszted, mentés.
Egy próbakör: impactshop巡_RUN() (vagy időzített triggerrel futtatod).
Ha a logban még mindig sok a „TIME-GUARD”, emelj 10–20%-ot MAX_RUN_MS-en vagy csökkents SHOPS_PER_RUN-t kicsit.
Ha ritkán „HTTP …” hibák látszanak, tartsd magasabban a SLEEP_BETWEEN-t (pl. 100–120 ms), hogy kíméletesebb legyen.
Így nagyobb boltmennyiségnél is stabilan, körökre bontva halad tovább a Patrol, és nem áll meg idő előtt. Következő lépésként érdemes egy futási statot figyelni (feldolgozott boltok / kör, hibaarány), hogy lásd, kell-e még finomítani.
