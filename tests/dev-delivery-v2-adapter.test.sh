...
t="$(mktemp -d)"; trap 'rm -rf "$t"' EXIT; mkdir -p "$t/config" "$t/scripts"; cp "$ROOT/config/dev-delivery-v2-target-contract.json" "$ROOT/config/dev-delivery-v2-impact-policy.json" "$t/config/"; cp "$A" "$t/scripts/"; git -C "$t" init -q; git -C "$t" config user.email t@x; git -C "$t" config user.name t; git -C "$t" add .; git -C "$t" commit -qm x; git -C "$t" branch -M main; git -C "$t" checkout -qb f
