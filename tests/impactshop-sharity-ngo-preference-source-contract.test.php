...
assert(sharity_ngo_pref_issue_code($codes, $raw_code, $claims, 10));
assert(sharity_ngo_pref_redeem_code($codes, $raw_code, 'wrong-grant', 'bff-prod', 'https://bff.example/callback', 'verifier', 20) === null);
assert(sharity_ngo_pref_redeem_code($codes, $raw_code, 'grant', 'bff-prod', 'https://bff.example/callback', 'verifier', 20) === null);
assert(sharity_ngo_pref_issue_code($codes, str_repeat('D', 32), $claims, 10));
assert(sharity_ngo_pref_redeem_code($codes, str_repeat('D', 32), 'grant', 'bff-prod', 'https://bff.example/callback', 'verifier', 20) === 'v1:subject');
assert(sharity_ngo_pref_redeem_code($codes, str_repeat('D', 32), 'grant', 'bff-prod', 'https://bff.example/callback', 'verifier', 20) === null);
assert(sharity_ngo_pref_response_headers()['Cache-Control'] === 'private, no-store');
assert(count(sharity_ngo_pref_table_names()) === 5);
assert(sharity_ngo_pref_table_names()[0]['writes'] === ['issue', 'consume']);
...
$idempotency = [];
$audit = [];
assert(sharity_ngo_pref_cas_update($preferences, $idempotency, $audit, 'v1:subject', 'profile_default', 2, $revision, $revision, 'selectable', 0, 'op-1', 20) === 'updated');
assert(sharity_ngo_pref_cas_update($preferences, $idempotency, $audit, 'v1:subject', 'profile_default', 2, $revision, $revision, 'selectable', 0, 'op-1', 21) === 'idempotent_replay');
assert(sharity_ngo_pref_cas_update($preferences, $idempotency, $audit, 'v1:subject', 'profile_default', 1, $revision, $revision, 'selectable', 1, 'op-1', 21) === 'idempotency_conflict');
assert(sharity_ngo_pref_cas_update($preferences, $idempotency, $audit, 'v1:subject', 'profile_default', 1, 'stale', $revision, 'selectable', 1, 'op-2', 21) === 'stale_revision');
assert(sharity_ngo_pref_cas_update($preferences, $idempotency, $audit, 'v1:subject', 'profile_default', 1, $revision, $revision, 'selection_required', 1, 'op-3', 21) === 'selection_required');
assert(count($audit) === 1);
assert($audit[0]['before_ngo_id'] === null && $audit[0]['after_ngo_id'] === 2);
