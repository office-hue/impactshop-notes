...
export function verifyStageA({ contract, capabilities, bastion } = {}) {
  try {
    ...
    if (!bastion.forbiddenOperations.includes('provider') || !bastion.forbiddenOperations.includes('deploy') || !bastion.forbiddenOperations.includes('vps') || !bastion.forbiddenOperations.includes('secret')) return fail('bastion_scope_incomplete');
